  ...
  describe '#perform' do
    ...
    context 'when pricing plan is community' do
      ...
      it 'will not create a premium config reset warning if config is not modified' do
        create(:installation_config, name: 'INSTALLATION_NAME', value: 'TwoBrain')
        service.perform
      ...
      it 'updates the premium configs to default' do
        ...
        service.perform
        expect(InstallationConfig.find_by(name: 'INSTALLATION_NAME').value).to eq('TwoBrain')
        expect(InstallationConfig.find_by(name: 'LOGO').value).to eq('/brand-assets/twobrain-logo.svg')
      end
    end
